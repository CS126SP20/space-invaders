...
//! the radius in physics dims is sometimes less than 1, so the number of circle
//! segments needs to be chosen manually.
...
BodyRef makeBodyShared(b2World *world, const b2BodyDef &bodyDef) {
  return BodyRef(world->CreateBody(&bodyDef),
                 [world](b2Body *body) { world->DestroyBody(body); });
}

auto toCinder(const b2Vec2 &vec) -> vec2 { return vec2(vec.x, vec.y); }
auto toCinder(const b2Color &color) -> Color {
...
bool DebugDraw::isFlagBitEnabled(uint32 bit) const { return GetFlags() & bit; }
...
void DebugDraw::DrawPolygon(const b2Vec2 *vertices, int32 vertexCount,
  ...
  for (size_t i = 1; i < vertexCount; i++) path.lineTo(toCinder(vertices[i]));
...
void DebugDraw::DrawTransform(const b2Transform &xf) {
  gl::color(1, 1, 0);
  gl::drawLine(toCinder(xf.p),
               toCinder(xf.p + TRANSFORM_AXIS_SCALE * xf.q.GetXAxis()));

  gl::color(0, 1, 1);
  gl::drawLine(toCinder(xf.p),
               toCinder(xf.p + TRANSFORM_AXIS_SCALE * xf.q.GetYAxis()));
}
